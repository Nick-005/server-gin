    ...
    "paths": {
        ...
        "/auth/user": {
            "post": {
                ...
                "parameters": [
                    {
                        "description": "Актуальные логин (ИНН?) и пароль работодателя",
                        "name": "ДанныеРаботодателя",
                        "in": "body",
                        "required": true,
                        "schema": {
                ...
                "responses": {
                    "200": {
                        "description": "Возвращает актуальный и новый токен для работодателя. Если произошла ошибка - статус будет 'Err' и будет возвращен текст ошибки! Также будет известно, где именно произошла ошибка!",
                        "schema": {
                            "$ref": "#/definitions/main.TokenForUser"
                        }
                    },
                    "400": {
                        "description": "Возвращает ошибку, если не удалось распарсить body, который отвечает за данные работодателя!",
                        "schema": {
                            "$ref": "#/definitions/main.InfoError"
                        }
                    },
                    "401": {
                        "description": "Возвращает ошибку, если не удалось найти работодателя в БД, который соответствовал бы данным, которые были получены сервером в результате этого запроса!",
                        "schema": {
